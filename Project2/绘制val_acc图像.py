...
in_log_path = './自带+几个改动/实验一_自带的.txt'  # 输入日志文件的位置
out_fig_path = './自带+几个改动/实验二.jpg'  # 输出图片的位置
...
EPOCH = 0
target_str = ['Validation Accuracy = ', 'EPOCH ']
while True:
    line = f.readline()
    if len(line) < 1:
        break
...
# in_log_path2 = './自带+几个改动/实验二_加一层卷积层.txt'  # 输入日志文件的位置
# out_fig_path2 = './自带+几个改动/实验二_加一层卷积层.jpg'  # 输出图片的位置
...
in_log_path3 = './自带+几个改动/实验三_加一层全连接层.txt'  # 输入日志文件的位置
out_fig_path3 = './自带+几个改动/实验三_加一层全连接层.jpg'  # 输出图片的位置
...
ax1.plot(EPOCH * np.arange(len(accuracy_test1)), accuracy_test1, color='g', label='自带的', linestyle='-')

# # 绘制实验二曲线图像，颜色为黄色'y'
# ax1.plot(EPOCH * np.arange(len(accuracy_test2)), accuracy_test2, color='y', label='加一层卷积层', linestyle='-')

#绘制实验三曲线图像，颜色为红色'r'
ax1.plot(EPOCH * np.arange(len(accuracy_test3)), accuracy_test3, color='r', label='加一层全连接层', linestyle='-')
