...
in_log_path = './实验五.txt'  # 输入日志文件的位置
out_fig_path = './实验_maxpooling.jpg'  # 输出图片的位置
...
in_log_path2 = './实验六.txt'  # 输入日志文件的位置
out_fig_path2 = './实验六.jpg'  # 输出图片的位置
...
ax1.plot(EPOCH * np.arange(len(accuracy_test1)), accuracy_test1, color='g', label='无maxpooling', linestyle='-')

# 绘制实验二曲线图像，颜色为黄色'y'
ax1.plot(EPOCH * np.arange(len(accuracy_test2)), accuracy_test2, color='y', label='maxpooling(2,2)', linestyle='-')
#
# # 绘制实验三曲线图像，颜色为红色'r'
